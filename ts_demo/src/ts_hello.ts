...
export function demo08(): void {
  ...
  const animal: Animal = Animal.Cat;
  console.log(animal);
  const animalName = Animal[4];
...
export function demo10(): void {
  // 联合类型
  const foo: string | number = "typescript";
  const answer: string = foo as string;
  console.log(answer);
}
...
export function demo15(): void {
  ...
  const myArray: StringArray = ["js", "ts", "java"];
  console.log(myArray);
}
...
export function demo16(): void {
  ...
  const sum: Sum = function (num1, num2) {
    return num1 + num2;
  };
...
export function demo19(): void {
  ...
  const cube: Cube = new Cube(2, 3, 4);
...
export function demo31(): void {
  ...
  const hello: iFoo<string> = (name) => "hello: " + name;
...
export function demo32(): void {
  ...
  const rect4 = new Rect4<number>(2, 3);
