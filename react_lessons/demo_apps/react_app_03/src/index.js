...
const Challenges = (props) => {
  console.log('Challenges Component context:', JSON.stringify(props, null, 2))
  const path = props.location.pathname
  const items = path.split('/')
  const slug = items.slice(items.length - 1)[0]
...
const User = ({ match, isLoggedIn, handleLogin }) => {
  console.log('User Component context:', JSON.stringify(match, null, 2))
